    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          console.log('[WHISPER] Sending audio to transcribeAudioV5, blob size:', blob.size, 'base64 length:', audioBase64.length);
          let response;
          try {
            response = await base44.functions.invoke('transcribeAudioV5', { audio: audioBase64 });
          } catch (e) {
          ...
          // Relancer avec délai pour respecter le rate limit (3 req/min = 20s entre envois)
          if (activeRef.current && !submittedRef.current) {
      ...
      // Enregistrer pendant 2-3 secondes pour transcription rapide
      setTimeout(() => {
        if (mediaRecorder.state === 'recording') {
          console.log('[WHISPER] Stopping recording after 2.5s');
          mediaRecorder.stop();
        } else {
