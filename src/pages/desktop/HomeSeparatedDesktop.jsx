...
import FileUploaderSeparatedDesktop from '@/components/upload/FileUploaderSeparatedDesktop';
import ParseProgressSeparatedDesktop from '@/components/upload/ParseProgressSeparatedDesktop';
import ScriptSummarySeparatedDesktop from '@/components/upload/ScriptSummarySeparatedDesktop';
import { parseScriptWithLLMSeparatedDesktop, verifyScriptIntegritySeparatedDesktop } from '@/lib/scriptParserSeparatedDesktop';
import { toast } from 'sonner';
import BottomNav from '@/components/BottomNav';
import { useAuth } from '@/lib/AuthContext';

export default function HomeSeparatedDesktop() {
  ...
  const handleFileUploaded = async (url, uploadedFileName) => {
    ...
    try {
      const result = await parseScriptWithLLMSeparatedDesktop(url, uploadedFileName, (progressValue) => {
      ...
      console.log(`[HomeSeparatedDesktop] Parsing completed in ${duration}s`, result);
      ...
      const integrity = await verifyScriptIntegritySeparatedDesktop(result.rawText || '', parsed);
      ...
      const duration = ((Date.now() - startTime) / 1000).toFixed(1);
      console.error(`[HomeSeparatedDesktop] Erreur analyse après ${duration}s:`, err);
  ...
  return (
    <div className="min-h-screen flex flex-col pb-20 overflow-x-hidden">
      <header className="px-6 py-4 border-b border-border/50">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-foreground">SOUFFLEUR [DESKTOP ISOLATED]</h1>
          </div>
          <div className="flex items-center gap-3">
            {!isAndroid && (
      ...
      {(isProcessing || processingError) && <ParseProgressSeparatedDesktop fileName={fileName} progress={progress} logs={logs} error={processingError} />}
          ...
          <div className="space-y-6">
            ...
            {step === 'upload' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  <FileUploaderSeparatedDesktop onFileUploaded={handleFileUploaded} isProcessing={isProcessing} progress={progress} />
              </motion.div>
            )}

            {step === 'summary' && parsedScript && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                 <ScriptSummarySeparatedDesktop 
