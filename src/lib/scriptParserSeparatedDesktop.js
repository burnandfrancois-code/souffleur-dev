...
export async function parseScriptWithLLMSeparatedDesktop(fileUrl, fileName, onProgress, onLogs) {
  try {
    console.log(`[parseScriptWithLLMSeparatedDesktop] Starting parse for: ${fileName}`);
    console.log(`[parseScriptWithLLMSeparatedDesktop] File URL: ${fileUrl}`);
    onProgress?.(10);
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        if ((status === 502 || status === 503 || status === 504 || isTimeout) && attempt < maxAttempts - 1) {
          ...
          console.warn(`[parseScriptWithLLMSeparatedDesktop] Error ${status || 'TIMEOUT'}, retrying in ${delayMs}ms (attempt ${attempt}/${maxAttempts - 1})`);
    ...
    console.log(`[parseScriptWithLLMSeparatedDesktop] Parse successful: ${responseData?.characters?.length} characters, ${responseData?.lines?.length} lines`);
    ...
    console.error('[parseScriptWithLLMSeparatedDesktop] Parse failed:', error);
    
    // Extract backend error message if available
    const backendError = error?.response?.data?.error || error?.message || 'Unknown error';
    console.error('[parseScriptWithLLMSeparatedDesktop] Backend error:', backendError);
    
    throw new Error(backendError);
  }
}

export async function verifyScriptIntegritySeparatedDesktop(rawText, parsedScript) {
  try {
...
export async function compareTextsSeparatedDesktop(expectedText, spokenText) {
  try {
