...
const MyLineRecorderV2 = forwardRef(function MyLineRecorderV2({ line, script, myCharacter, onLineAdvance }, ref) {
  ...
  // ── Commandes vocales en phase résultat ──────────────────────────────────
  ...
  // Lancer la commande vocale dès qu'on entre en phase résultat
  useEffect(() => {
    if (phase === 'result') {
  ...
  const handleContinue = useCallback(() => {
    ...
    onLineAdvance?.();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onLineAdvance, stopCommandListening]);
  ...
  useEffect(() => {
    ...
    if (autoPlayRef.current && !trainingMode) {
      ...
      const timer = setTimeout(() => { startRecordingRef.current?.(); }, delay);
      return () => clearTimeout(timer);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [line, trainingMode]);
  ...
  const handleMicToggle = () => {
    if (isRecording) voiceRec.stop();
    else startRecording();
  };

  const handleReset = () => {
    voiceRec.reset();
    if (autoPlayRef.current && !trainingMode) setTimeout(() => startRecording(), 300);
  };
      ...
      <div className="flex-1 min-w-0 max-w-[90%]">
        ...
        <div className="flex justify-center mb-3">
          <Button
            ...
            onClick={async () => {
              ...
              if (isRecording) { voiceRec.stop(); await new Promise(r => setTimeout(r, 350)); }
              setIsSpeakingMyLine(true);
              await speakText(line.text, 'fr-FR', 'male', speechRateRef.current);
              setIsSpeakingMyLine(false);
              if (wasRecording && autoPlayRef.current) setTimeout(() => startRecording(), 200);
            }}
        ...
        {voiceRec.error && (
          ...
          >
            ...
            <div className="text-sm text-destructive whitespace-pre-wrap">{voiceRec.error.message}</div>
          </motion.div>
        )}

        {/* Recording banner */}
        {phase === 'line' && (
          <div className={`rounded-2xl border-2 p-5 mb-3 text-center transition-all duration-300 ${isRecording ? 'border-destructive bg-destructive/10' : 'border-primary bg-primary/10'}`}>
            ...
            <button
              ...
              className={`relative w-24 h-24 rounded-full mx-auto flex items-center justify-center transition-all ${isRecording ? 'bg-destructive shadow-2xl shadow-destructive/50' : 'bg-primary shadow-xl shadow-primary/30 hover:scale-105'}`}
            >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {voiceRec.transcript && voiceRec.transcript !== '🎤' && !isRecording && !trainingMode && (
                <Button size="sm" onClick={() => handleSubmitRecording(voiceRec.transcript)} className="bg-primary text-primary-foreground">
...
export default MyLineRecorderV2;