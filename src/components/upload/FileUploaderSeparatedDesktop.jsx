...
export default function FileUploaderSeparatedDesktop({ onFileUploaded, isProcessing, progress }) {
  ...
  const handleFileSelect = async (file) => {
    ...
    console.log(`[FileUploaderSeparatedDesktop] Starting upload: ${file.name} (${file.size} bytes, ${file.type})`);
    ...
    try {
      ...
      console.log(`[FileUploaderSeparatedDesktop] Upload result:`, result);
      
      if (!result?.file_url) {
        throw new Error('No file_url returned from upload');
      }
      
      console.log(`[FileUploaderSeparatedDesktop] Upload successful: ${result.file_url}`);
      toast.dismiss();
      onFileUploaded(result.file_url, file.name);
    } catch (error) {
      console.error('[FileUploaderSeparatedDesktop] Upload failed:', error);
      toast.dismiss();
